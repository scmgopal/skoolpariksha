		...
		var imgDir = "/html/skin/image/common/poll/images/"	// directory for images ... e.g. var imgDir="/easWeb/resources/images"
	    ...
	    */
		...
		function closeCalendar() {
			...
			oEvent.prevDate = ctlToPlaceValue.value; 
			ctlToPlaceValue.value =	constructDate(dateSelected,monthSelected,yearSelected);
			oEvent.newDate = ctlToPlaceValue.value; 
			oEvent.fired=true;
			
			// To set date of resultDeclarationDate three weeks ahead and it is specific to groups portlet only
			if(document.getElementById("resultDeclarationDate") && document.getElementById("dateOfTest") == ctlToPlaceValue)
				{
					var resDecDate = ctlToPlaceValue.value;
					var year1  = parseFloat(resDecDate.substring (0, resDecDate.indexOf ("-")));
					var month1 = parseFloat(resDecDate.substring (resDecDate.indexOf ("-")+1, resDecDate.lastIndexOf ("-")));
					var date1 = parseFloat(resDecDate.substring (resDecDate.lastIndexOf ("-")+1, resDecDate.length));
					var currentTime = new Date() ;
					currentTime.setFullYear(year1,month1-1,date1);
					...
					document.getElementById("resultDeclarationDate").value = currentdate;
					
				}
			
		
			ctlToPlaceValue.fireEvent("onclick", oEvent);
		
			if (document.getElementById("callFromResReq")){
				
				if (ctlToPlaceValue.name == "expReqDateFrom" || ctlToPlaceValue.name == "expReqDateTo" ){
					onSubmit();
				}
		...
		function constructCalendar () {
			...
			for	( datePointer=1; datePointer<=numDaysInMonth; datePointer++ )
			{
				...
				sHTML += "<a id=id_calendarLink class='"+sStyle+"' "+dateMessage+" title=\"" + sHint + "\" href='javascript:dateSelected="+datePointer+";closePopupCalendar();'>&nbsp;" + datePointer + "&nbsp;</a>"
			...
			if (bPageLoaded)
			{
				if ( crossobj.visibility ==	"hidden" ) {
					...
					do {
                     ...
                     if((toppos+150 - document.body.scrollTop ) > document.body.offsetHeight)
                     {
                      toppos = toppos - 151;
                     }
                     
                     if((leftpos+228 - document.body.scrollLeft ) > document.body.offsetWidth)
                     {
                      leftpos = leftpos - 229 ;
                     }
	...
	/** This Method is uded for compare two dates with format (yyyy-mm-dd)
	 * 
	 *  Return 1   :- if value1 is greater than value2
	 *  Return -1  :- if value1 is lessthan than value2
